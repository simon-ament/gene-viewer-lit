...
@customElement('gene-list-autocomplete')
export class GeneListAutocomplete extends LitElement {
  ...
  static styles = css`
    ...
    .autocomplete-item.highlighted {
      background-color: color(
        from contrast-color(var(--background-color)) srgb r g b / 0.1
      );
    }
  ...
  protected _filterGeneList() {
    ...
    const filtered = this.geneList.filter(geneId =>
      geneId.toLowerCase().includes(lowerCaseInput),
    );
    ...
    this._inputElement.addEventListener('keydown', (event: KeyboardEvent) => {
      if (event.key === 'Enter') {
        if (
          ...
          this._highlightedIndex < this._filteredGeneList.length
        ) {
      ...
      if (event.key === 'ArrowDown' && this._filteredGeneList.length > 0) {
        ...
        this._highlightedIndex = Math.min(
          this._highlightedIndex + 1,
          this._filteredGeneList.length - 1,
        );
      }
    ...
    return html`
      <div class="autocomplete-wrapper">
        ...
        <ul
          ...
          .hidden="${this._filteredGeneList.length === 0}"
        >
          ${this._filteredGeneList.map(
            geneId =>
              html` <li
...
declare global {
  interface HTMLElementTagNameMap {
    'gene-list-autocomplete': GeneListAutocomplete;
  }
}
