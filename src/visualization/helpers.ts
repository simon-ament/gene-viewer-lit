 ...
 */
export const probeTooltipHTML = (probe: Probe, regions: Regions) => {
  ...
  return (
    ...
    (matchingTranscripts !== 0
      ? `<br>Covered transcripts: ${
          matchingTranscripts < 5
            ? probe.transcript_ids.join(', ')
            : `Probe covers ${matchingTranscripts} of ${totalTranscripts} transcripts`
        }`
      : '')
  );
};
...
export const regionTooltipHTML = (
  ...
  (transcriptName !== 'unknown' ? `<br>Transcript: ${transcriptName}` : '');
 ...
 * @returns A string containing HTML content for the tooltip.
 */
export const transcriptTooltipHTML = (transcriptId: string, probes: Probes) => {
  const matchingProbes = Object.values(probes)
    .flat()
    .filter(probe => probe.transcript_ids.includes(transcriptId));
  const uniqueProbeIds = Array.from(new Set(matchingProbes.map(o => o.id)));
  return `Transcript: ${transcriptId}${
    uniqueProbeIds.length > 0 ? '<br>Probes: ' : ''
  }${
    uniqueProbeIds.length < 5
      ? uniqueProbeIds.join(', ')
      : `${uniqueProbeIds.length} probes cover this transcript`
  }`;
};
...
export const collectReferenceBases = (
  ...
  const visibleSequences = gene.sequences.filter(
    sequence =>
      sequence.start <= end &&
      sequence.start + sequence.sequence.length >= start,
  );
  ...
  visibleSequences.forEach(sequence => {
    ...
    for (
      let pos = regionStartPos;
      pos <= sequence.start + sequence.sequence.length && pos <= end;
      pos += 1
    ) {
      bases.push({
...
const complementBase = (base: string): string => {
  switch (base.toUpperCase()) {
    ...
    case 'G':
      return base === 'G' ? 'C' : 'c';
    default:
      return base; // Return the base as is if it's not A, T, C, or G
  }
};
...
export const collectProbeBases = (
  ...
  const visibleProbes = probes.filter(
    probe => probe.end >= start && probe.start <= end,
  );
  ...
  visibleProbes.forEach(probe => {
    ...
    for (let pos = probeStartPos; pos <= probe.end && pos <= end; pos += 1) {
      const referenceBase = referenceBases.find(base => base.position === pos);
      if (referenceBase) {
        bases.push({
          char: complementBase(referenceBase.char),
          position: pos,
        });
      }
    }
...
export const exportSVG = (el: SVGElement, filename: string) => {
  ...
  cssVariables.forEach(variable => {
    ...
    svgStringWithComputedStyles = svgStringWithComputedStyles.replace(
      regex,
      value,
    );
  });
  ...
  const blob = new Blob([svgStringWithComputedStyles], {
    type: 'image/svg+xml',
  });
