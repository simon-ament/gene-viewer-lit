...
import { drawHeader, drawFooter } from './legend.js';
import { updateLocationIndicator } from './location-indicator.js';
...
const createContext = (
  ...
  const svg = d3.select(el).select('svg') as d3.Selection<
    SVGElement,
    unknown,
    null,
    unknown
 ...
 * @param gene The gene data containing regions and probes to be visualized.
 */
const setupScalesAndAxes = (context: VisualizationContext, gene: Gene) => {
  // Define x scale based on combined extent of probes and genomic regions
  const ext = d3.extent([
    ...Object.values(gene.probes)
      .flat()
      .flatMap((d: Probe) => [d.start, d.end]),
    ...Object.values(gene.regions)
      .flat()
      .flatMap((d: Region) => [d.start, d.end]),
    ...Object.values(gene.sequences)
      .flat()
      .flatMap((d: Sequence) => [d.start, d.start + d.sequence.length - 1]),
    ...Object.values(gene.tracks)
      .flat()
      .flatMap((d: Feature) => [d.start, d.end]),
  ]) as [number, number];
 ...
 * @param gene The gene data containing regions and probes to be visualized.
 */
const setupElements = (context: VisualizationContext, gene: Gene) => {
  context.svg
  ...
  Object.entries(gene.regions).forEach(([transcriptName, regions]) => {
    ...
    const regionsContainer = transcriptGroup
      ...
      .attr(
        'transform',
        (d: Region) => `translate(${context.xScale(d.start - 0.5)}, 0)`,
      )
      .on('mouseover', (_, d: Region) => {
        context.tooltip
      ...
      .on('mousemove mousemove-forwarded', event => {
        const xPos =
          event instanceof MouseEvent ? event.pageX : event.detail.pageX;
        const yPos =
          event instanceof MouseEvent ? event.pageY : event.detail.pageY;
        context.tooltip
          .style('left', xPos > window.innerWidth / 2 ? '' : `${xPos + 20}px`)
          .style(
            'right',
    ...
    regionsContainer
      ...
      .attr('height', (d: Region) =>
        d.type === 'intron' ? TRANSCRIPT_HEIGHT / 10 : TRANSCRIPT_HEIGHT / 2,
      )
      .attr('x', 0)
      .attr('y', function () {
    ...
    context.svg
      ...
      .text(
        transcriptName === 'unknown'
          ? ''
          : transcriptName.slice(0, 8) +
              (transcriptName.length > 8 ? '...' : ''),
      ) // truncate long names
...
const setupMouseEvents = (
  ...
  const preventPageScroll: (event: WheelEvent) => void = event => {
...
const zoomed = (
  ...
  if (event.sourceEvent instanceof MouseEvent && plotNode) {
    ...
    if (event.sourceEvent.type === 'mousemove') {
      ...
      if (root instanceof ShadowRoot) {
        const hoveredElement = root.elementFromPoint(
  ...
  const bases = showBases
    ? collectReferenceBases(gene, Math.floor(domain[0]), Math.ceil(domain[1]))
    : [];
  context.baseGroup
  ...
  const probeBases = collectProbeBases(
    Object.entries(gene.probes)
      ...
      .flat()
      .filter(probe =>
        selection.probeIds.length > 0
          ? selection.probeIds.includes(probe.id)
          : true,
      ),
    bases,
    Math.floor(domain[0]),
  ...
  if (showArrows) {
    ...
    const visibleRegions = regions.filter(
      d => d.end >= domain[0] && d.start <= domain[1] && d.strand !== undefined,
    );
    visibleRegions
...
const setupZoom = (
  ...
  context.zoomBehavior
    .scaleExtent([
      1,
      (context.xScale.domain()[1] - context.xScale.domain()[0]) /
        (80 / context.scaleFactor),
    ]) // max zoom to 80bp width on default scale
...
class GeneViewerVisualization {
  ...
  constructor(
    ...
    selection: ProbeSelection,
    setSelection: (s: ProbeSelection, zoomIntoProbes?: boolean) => void,
    parallelProbesets: number,
    scaleFactor: number,
    regionMap: RegionMap,
  ) {
    ...
    this.parallelProbesets = Math.min(
      parallelProbesets,
      Object.keys(gene.probes).length,
    );
  ...
  public showProbesets(visibleProbesets: string[]) {
    ...
    probesetTracks
      ...
      .attr('stroke', d =>
        this.selection.probesetId === d.probesetId
          ? 'orange'
          : 'var(--border-color)',
      )
      .attr('stroke-width', 1)
    ...
    probesetTracks
      ...
      .attr(
        'width',
        d =>
          this.context.xScale(d.end + 0.5) - this.context.xScale(d.start - 0.5),
      )
    ...
    probesetTracks
      ...
      .on('click', (event, data) => {
        ...
        this.setSelection({ probesetId: null, probeIds: [data.id] }, true); // zoom into selected probe (even if already selected)
      })
    ...
    this.context.svg
      ...
      .attr('cursor', 'pointer')
      .attr('fill', d =>
        this.selection.probesetId === d ? 'orange' : 'var(--text-color)',
      )
      ...
      .on('click', (event, d) => {
        ...
        this.setSelection({ probesetId: d, probeIds: probesInSet }, true); // zoom into selected probeset
      });
    ...
    zoomed(
      { transform: this.context.currentZoomTransform } as d3.D3ZoomEvent<
        SVGGElement,
        unknown
      >,
      this.context,
  ...
  public select(selection: ProbeSelection, zoomIntoProbes: boolean = false) {
    ...
    this.context.probesGroup
      ...
      .attr('stroke', d =>
        selection.probesetId === d.probesetId
          ? 'orange'
          : 'var(--border-color)',
      );
    ...
    this.context.svg
      ...
      .attr('fill', d => {
        if (selectedProbes.length > 0) {
          const isSelected = selectedProbes.some(probe =>
            probe.transcript_ids.includes(d),
          );
    ...
    if (selectedProbes.length > 0 && zoomIntoProbes) {
      const selectionStart = Math.min(
        ...selectedProbes.map(probe => probe.start),
      );
